...
import {Config, IAbstractDiscoveryService} from "../types";


export class AbstractDiscoveryService implements IAbstractDiscoveryService {
  ...
  public async init(): Promise<boolean> {
    ...
    this._SERVER_TAG = process.env.SERVER_TAG ?? 'default_01';
    ...
    try {
      ...
      if ('web-client' in config) {
        await fse.writeFile(
          ...
          `NEXT_PUBLIC_CLIENT_ENVS=${JSON.stringify(config['web-client'])}`
        );
        delete config['web-client'];
      }
